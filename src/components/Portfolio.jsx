  ...
  return (
    ...
    >
        ...
        {projectLink ? (
          <a href={projectLink} target="_blank" rel="noopener noreferrer">
            <img
              src={image}
              alt='project_image'
              className='w-full h-auto object-cover md:rounded-3xl cursor-pointer hover:opacity-90 transition-opacity'
            />
          </a>
        ) : (
          <img
      ...
      <div className={`w-full md:w-2/5 px-6 md:p-16 flex flex-col justify-center ${isEven ? "text-left md:text-left" : "text-left md:text-right"}`}>
        ...
        {projectLink && (
          <p className='mt-2 text-quaternary text-xs sm:text-xs md:text-sm italic'>
            Click on the image to view the project on GitHub
          </p>
        )}
      </div>
    </motion.div>
